...
import { ShieldCheck, AlertTriangle, Sparkles, Building2, DollarSign, Award, Users } from 'lucide-react';
...
export const CredibilityRadar: React.FC<Props> = ({ claims, disputes }) => {
  ...
  const venueClaims = claims.filter(c => c.researchDomain === 'FESTIVAL' || c.category.toLowerCase().includes('venue') || c.category.toLowerCase().includes('screening'));
  const feeClaims = claims.filter(c => c.category.toLowerCase().includes('fee') || c.category.toLowerCase().includes('prize'));
  const organizerClaims = claims.filter(c => c.researchDomain === 'ORGANIZER' || c.category.toLowerCase().includes('jury') || c.category.toLowerCase().includes('company'));
  const participantClaims = claims.filter(c => c.researchDomain === 'PARTICIPANTS' || c.category.toLowerCase().includes('feedback'));
  ...
  const calcScore = (catClaims: AtomicClaim[], hasDispute: boolean) => {
    ...
    let score = Math.round(((corroborated * 1.0 + supported * 0.8) / Math.max(1, catClaims.length)) * 100);
    if (hasDispute || disputed > 0) score = Math.min(score, 45);
    return Math.max(25, Math.min(100, score));
  };

  const hasVenueDispute = disputes.some(d => d.pointOfContention.toLowerCase().includes('venue') || d.pointOfContention.toLowerCase().includes('screening') || d.category.toLowerCase().includes('venue'));
  const hasFeeDispute = disputes.some(d => d.pointOfContention.toLowerCase().includes('fee') || d.pointOfContention.toLowerCase().includes('prize') || d.category.toLowerCase().includes('fee'));
  const hasOrganizerDispute = disputes.some(d => d.pointOfContention.toLowerCase().includes('organizer') || d.pointOfContention.toLowerCase().includes('company') || d.category.toLowerCase().includes('organizer'));
  const hasParticipantDispute = disputes.some(d => d.pointOfContention.toLowerCase().includes('feedback') || d.pointOfContention.toLowerCase().includes('participant') || d.category.toLowerCase().includes('community'));
  ...
  return (
    <div className="p-6 rounded-2xl bg-paper-surface dark:bg-darkroom-surface border border-paper-border dark:border-darkroom-border space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-paper-border dark:border-darkroom-border pb-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-paper-muted dark:text-darkroom-muted">Overall Index:</span>
            ...
            >
              ...
              <div className="flex items-center justify-between text-[11px] font-mono">
                <span className={`inline-flex items-center gap-1 ${
                  dim.isRisk ? 'text-rose-600 dark:text-rose-400' : 'text-paper-muted dark:text-darkroom-muted'
                }`}>
                  {dim.isRisk ? <AlertTriangle className="size-3 shrink-0" /> : <ShieldCheck className="size-3 shrink-0 text-emerald-500" />}
