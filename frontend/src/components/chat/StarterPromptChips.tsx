...
export const StarterPromptChips: React.FC<StarterPromptChipsProps> = ({ onSelectPrompt }) => {
  return (
    <div className="w-full max-w-3xl mx-auto mt-6">
      <p className="text-xs font-mono font-semibold uppercase tracking-wider text-zinc-400 text-center mb-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {STARTER_PROMPTS.map((item, idx) => (
          <button
            ...
            className="flex items-start gap-3.5 p-3.5 text-left rounded-2xl border border-zinc-800/80 bg-zinc-900/40 hover:bg-zinc-850 hover:border-amber-500/40 transition-all group backdrop-blur-sm cursor-pointer"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="text-sm font-semibold text-zinc-200 group-hover:text-amber-400 transition-colors truncate">
                  {item.title}
                </span>
                <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 group-hover:bg-amber-500/20 group-hover:text-amber-300">
                  {item.badge}
                </span>
              </div>
