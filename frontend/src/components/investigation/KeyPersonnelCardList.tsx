...
export const KeyPersonnelCardList: React.FC<Props> = ({
  ...
  subtitle = 'Verified festival organizers, committee leaders, and connected personnel evaluated for conflicts of interest.',
}) => {
  ...
  const visibleLeadership = showAllLeadership ? leadership : leadership.slice(0, 6);

  const renderPersonCard = (person: KeyPerson, idx: number) => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
          ...
          <div className="border-b border-darkroom-border pb-2.5 flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
              <span className="text-xs font-mono font-semibold text-tool-diligence bg-tool-diligence/10 border border-tool-diligence/20 px-2.5 py-0.5 rounded-full">
                {leadership.length} Core Organizers
              </span>
              {connected.length > 0 && (
                <span className="text-xs font-mono text-slate-400 bg-darkroom-card border border-darkroom-border px-2.5 py-0.5 rounded-full">
        ...
        <div className="divide-y divide-darkroom-border/40 border-y border-darkroom-border/40 sm:border-y-0 sm:divide-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:gap-3.5">
          {visibleLeadership.map((person, idx) => renderPersonCard(person, idx))}
        </div>

        {/* Show all leadership toggle (cut at max 6) */}
        {leadership.length > 6 && (
          <button
            type="button"
      ...
      {/* Group 2: Connected Personnel, Academic & Extended Network */}
      {connected.length > 0 && (
        <div className="pt-2 border-t border-darkroom-border/60 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                University researchers, student liaisons, volunteers, and operational associates.
              </p>
            </div>
            <button
              type="button"
            ...
            >
              <span>{showConnected ? 'Hide' : 'View'} {connected.length} Connected People</span>
              {showConnected ? (
                <ChevronUp className="size-3.5 text-slate-400" />
              ) : (
                <ChevronDown className="size-3.5 text-slate-400" />
          ...
          {showConnected && (
            <div className="divide-y divide-darkroom-border/40 border-y border-darkroom-border/40 sm:border-y-0 sm:divide-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:gap-3.5 pt-2">
              {connected.map((person, idx) => renderPersonCard(person, idx))}
            </div>
          )}
        </div>
      )}
