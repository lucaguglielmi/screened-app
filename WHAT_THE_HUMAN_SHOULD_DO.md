...
| **HUMAN-08** | **Architecture Page** | Ensure the new graphical architecture diagram accurately aligns with the frontend `ArchitecturePage.tsx` live visualizer. | `COMPLETED` [2026-08-24] - Added PII and Telemetry layers. | Phase 4 (Production) |
| **HUMAN-09** | **GA4 Properties** | Update GA4 properties inside the Google Analytics console to create custom dimensions matching our new tracking schema. | `COMPLETED` [2026-08-24] - Created new property and added custom dimensions manually. | Phase 4 (Production) |
| **HUMAN-10** | **Diagnostics Token** | Provision the `DIAGNOSTICS_TOKEN` secret in Secret Manager and map it to the Cloud Run deployment. Without this, `/api/diagnostics` will perpetually return 404 in production. | `COMPLETED` [2026-08-25] - Provisioned `diagnostics-token` secret in Secret Manager, mapped it to Cloud Run deployment, and verified live 200 response with bearer token. | Operations |
| **HUMAN-11** | **IAM Grants for Telemetry** | Grant the necessary IAM roles for Cloud Trace, Cloud Logging, and Error Reporting to the Cloud Run service account to enable the observability stack. | `COMPLETED` [2026-08-25] - Granted `roles/cloudtrace.agent`, `roles/logging.logWriter`, `roles/errorreporting.writer`, and `roles/monitoring.metricWriter` to compute service account (`786241671474-compute@developer.gserviceaccount.com`). | Operations |
